...
def modify_position(
	...
	*,
	...
	response = send_order(
		...
		stop_loss = float(stop_loss) if stop_loss is not None else float(position["stop_loss"]),
		take_profit = float(take_profit) if take_profit is not None else float(position["take_profit"]),
	)
