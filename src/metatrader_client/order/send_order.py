...
def send_order(
	...
	*,
	...
	# Validate TP and SL — always cast to native float so numpy scalars
	# (e.g. np.float64 read from a positions DataFrame) never leak into the
	# RPyC eval string as `np.float64(...)` on the remote terminal side.
	try:
	...
	match action:
		...
		case TradeRequestActions.DEAL:
			...
			# order_send อาจ "ส่งได้" (last_error=0) แต่ server REJECT (retcode ≠ DONE/PLACED) →
			# success=True + order id ของไม้ที่ไม่เกิดจริง (phantom). เช็ค retcode กันไว้.
			if response is None or response.retcode not in (
					mt5.TRADE_RETCODE_DONE, mt5.TRADE_RETCODE_PLACED, mt5.TRADE_RETCODE_DONE_PARTIAL):
				rc = getattr(response, "retcode", None); cm = getattr(response, "comment", "")
				return { "success": False, "message": f"Order rejected: retcode={rc} {cm}".strip(), "data": response }
		...
		case TradeRequestActions.PENDING:
			...
			if error_code < 0:
				return { "success": False, "message": f"Error {error_code}: {error_description}", "data": None }
			# order_send อาจ "ส่งได้" (last_error=0) แต่ server REJECT (retcode ≠ DONE/PLACED เช่น
			# market closed/invalid price/requote ช่วง rollover). ถ้าไม่เช็คจะคืน success=True +
			# OrderSendResult.order ของออเดอร์ที่ไม่เกิดจริง (phantom). เช็ค retcode กันไว้.
			if response is None or response.retcode not in (
					mt5.TRADE_RETCODE_DONE, mt5.TRADE_RETCODE_PLACED, mt5.TRADE_RETCODE_DONE_PARTIAL):
				rc = getattr(response, "retcode", None); cm = getattr(response, "comment", "")
				return { "success": False, "message": f"Order rejected: retcode={rc} {cm}".strip(), "data": response }
