...
@mcp.tool()
def close_position(ctx: Context, id: Union[int, str], volume: Optional[Union[int, float]] = None,
		comment: Optional[str] = "") -> dict:
	"""Close an open position by ID, fully or partially. Parameters:
		id: Position ID.
		volume (optional): Lots to close. Omit to close the full position;
			pass a smaller value than the position volume for a partial close.
		comment (optional): Comment on the closing deal. On a partial close the
			broker may overwrite the remaining position's comment with this — pass
			the owning system tag (e.g. "ken") to keep the remainder attributable.
	"""
	client = get_client(ctx)
	return client.order.close_position(id=id, volume=volume, comment=comment)
