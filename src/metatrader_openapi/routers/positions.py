from fastapi import APIRouter, HTTPException, Request, Path, Body, Query
...
async def close_position(
    request: Request,
    id: Union[int, str] = Path(..., description="Position ID"),
    volume: Optional[float] = Query(None, description="Lots to close; omit to close the full position"),
):
    """Closes a specific position by its ID, fully or partially."""
    client = request.app.state.client
    try:
        return client.order.close_position(id=id, volume=volume)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
